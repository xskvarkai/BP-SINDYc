...
def find_noise(x: np.ndarray, detail_level: int = 1, verbose: bool = True) -> float:
    """
    Estimates the noise level in a signal based on the median absolute deviation of its Wavelet Denoising detail coefficients.
    """
...
def find_periodicity(
    ...
    if sigma_noise > 0:
        noise_threshold = 3 * sigma_noise # 3-sigma pravidlo
...
def estimate_threshold(
    ...
    thresholds_non_rounded = np.logspace(np.log10(min_val), np.log10(max_val), n_threshold)
    thresholds = np.unique(np.round(thresholds_non_rounded, 8))
