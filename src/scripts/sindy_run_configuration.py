...
def run_config(configuration_and_data: List[Any]) -> Dict[str, Any]:
    try:
        ...
        filters_result = sindy_helpers.filter_model(model_sim, constraints, data, start_index, current_steps, {"rtol": 1e-3,"atol": 1e-3})
        if filters_result is not None: # Ak model neprejde filtrami, vratime informaciu o zlyhani a zahodime model bez dalsich simulacii a hodnoteni.
            return {"configuration": config, "error": filters_result}
