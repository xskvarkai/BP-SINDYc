...
from sysidentification import generate_input_signal, simulate, generate_noise

class DynamicSystem:
    def __init__(self, l=1.0, m=1.0, I = 1.0, k=1.0, b=0.0, g=9.81, omega_n=0.0):
        self.l = l
...
time_step = 0.1 # krok
time_span = (0, 20) # casovy interval
...
trajectory_free_body_RK4 = simulate(dynamic_system, input_free_body, initial_conditions, dt=time_step, integrator_method="RK4")
trajectory_free_body_Euler = simulate(dynamic_system, input_free_body, initial_conditions, dt=time_step, integrator_method="E")
trajectory_amprbs_RK4 = simulate(dynamic_system, input_amprbs, initial_conditions, dt=time_step, integrator_method="RK4")
trajectory_amprbs_Euler = simulate(dynamic_system, input_amprbs, initial_conditions, dt=time_step, integrator_method="E")
...
plt.subplot(2, 2, 2)
plt.plot(time_vector, trajectory_amprbs_RK4[:, 0], label='RK4 - AMPRBS', color='orange')
plt.plot(time_vector, trajectory_amprbs_RK4_noisy[:, 0], label='RK4 noisy - AMPRBS', linestyle='dashed', color='gold')
plt.title('Angle (theta) - AMPRBS')
plt.xlabel('Time (s)')
plt.ylabel('Angle (rad)')
plt.grid()
...
plt.subplot(2, 2, 4)
plt.plot(time_vector, trajectory_amprbs_RK4[:, 1], label='RK4 - AMPRBS', color='orange')
plt.plot(time_vector, trajectory_amprbs_RK4_noisy[:, 1], label='RK4 noisy - AMPRBS', linestyle='dashed', color='gold')
plt.title('Velocity (omega)')
plt.xlabel('Time (s)')
plt.ylabel('Velocity (rad/s)')
plt.grid()
